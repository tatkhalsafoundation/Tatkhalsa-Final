    ...
    <script>
      // Scrolling Logo Compression Logic
      const scrollingLogo = document.getElementById("scrolling-logo");
      const scrollingLogoWrapper = document.getElementById("scrollingLogoWrapper");
      
      function updateLogoSize() {
        if (!scrollingLogo || !scrollingLogoWrapper) return;
        const sY = window.scrollY || document.documentElement.scrollTop;
        const maxScroll = 120; // scroll distance for full shrinkage
        ...
        scrollingLogoWrapper.style.top = currentTop + "px";

        // Dynamic horizontal translation
        const leftOffset = isMobile ? 20 : Math.max(20, (window.innerWidth - 1200) / 2 + 20);
        const startLeft = window.innerWidth / 2;
        const endLeft = leftOffset + (currentSize / 2);
        const currentLeft = startLeft - pct * (startLeft - endLeft);
        ...
        if (logoEl) {
          const logoWidth = logoEl.offsetWidth || 180;
          const targetTranslation = (window.innerWidth / 2) - (leftOffset + logoWidth / 2);
          const currentTranslation = pct * targetTranslation;
          logoEl.style.transform = "translateX(" + currentTranslation + "px)";
