...
/**
 * Setup Admin Settings for SMS API (Fast2SMS)
 */
function tatkhalsa_sms_settings_init() {
    register_setting( 'general', 'tatkhalsa_fast2sms_api_key' );
    add_settings_field(
        'tatkhalsa_fast2sms_api_key',
        'Fast2SMS API Key (For OTPs)',
        'tatkhalsa_fast2sms_api_key_callback',
        'general',
        'default'
    );
}
add_action( 'admin_init', 'tatkhalsa_sms_settings_init' );

function tatkhalsa_fast2sms_api_key_callback() {
    $val = get_option( 'tatkhalsa_fast2sms_api_key' );
    echo '<input type="text" name="tatkhalsa_fast2sms_api_key" value="' . esc_attr($val) . '" class="regular-text" placeholder="Enter Fast2SMS API Key" />';
    echo '<p class="description">Required to send real SMS OTPs for Sevadar and Blood Donor verifications.</p>';
}

/**
 * Handle Sending OTP (Backend AJAX)
 */
function tatkhalsa_send_sms_otp() {
    ...
    // Check for API Key
    $api_key = get_option('tatkhalsa_fast2sms_api_key');
    if ( empty($api_key) ) {
        // Fallback for local development or if API is not set. We log it to error log instead of showing on screen.
        error_log("TATKHALSA OTP for $phone is: $otp");
        set_transient( 'otp_' . md5($phone), $otp, 5 * MINUTE_IN_SECONDS );
        
        // We will return a success but inform the developer through response (but not the actual OTP to user)
        wp_send_json_success( array('message' => 'API Key missing. OTP blocked from user. (Check PHP error logs for OTP)') );
    }
    ...
    $response = wp_remote_post("https://www.fast2sms.com/dev/bulkV2", array(
        ...
        'body' => json_encode(array(
            'route'     => 'v3',
            'sender_id' => 'TXTIND',
            'message'   => "Your Tatkhalsa Foundation Verification OTP is $otp. Valid for 5 mins.",
            'language'  => 'english',
            'flash'     => 0,
            'numbers'   => ltrim($phone, '+') // Remove + if present
        ))
    ));

    if ( is_wp_error($response) ) {
        wp_send_json_error('Failed to send SMS due to network error.');
    }
    ...
    if ( isset($body['return']) && $body['return'] === true ) {
        ...
        $msg = isset($body['message']) ? $body['message'] : 'SMS API Error';
        wp_send_json_error( "SMS Failed: " . $msg );
    }
}
add_action( 'wp_ajax_send_sms_otp', 'tatkhalsa_send_sms_otp' );
add_action( 'wp_ajax_nopriv_send_sms_otp', 'tatkhalsa_send_sms_otp' );

/**
 * Handle Verifying OTP (Backend AJAX)
 */
function tatkhalsa_verify_sms_otp() {
    $phone = sanitize_text_field( $_POST['phone'] );
    $otp = sanitize_text_field( $_POST['otp'] );

    if( empty($phone) || empty($otp) ) {
        wp_send_json_error( 'Phone and OTP are required.' );
    }

    $saved_otp = get_transient( 'otp_' . md5($phone) );

    if( $saved_otp && $saved_otp == $otp ) {
        delete_transient( 'otp_' . md5($phone) );
        wp_send_json_success( 'Phone verified successfully.' );
    } else {
        wp_send_json_error( 'Invalid or expired OTP.' );
    }
}
add_action( 'wp_ajax_verify_sms_otp', 'tatkhalsa_verify_sms_otp' );
add_action( 'wp_ajax_nopriv_verify_sms_otp', 'tatkhalsa_verify_sms_otp' );
